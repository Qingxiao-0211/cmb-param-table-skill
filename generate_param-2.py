...
def determine_detail_display_mode(std_data):
    ...
    规则：
    - 产品简称以"Y"结尾（Y类份额，养老专属）→ M:养老型模式
    - 产品名称包含"滚动持有期" → G:滚动持有期
    - 其他 → N:无
    """
    ...
    fund_short_name = str(fund_short_name).strip()
    
    # 产品简称以"Y"结尾 → 养老型模式（Y类份额是养老目标基金个人养老金专属份额）
    if fund_short_name.endswith('Y'):
        return 'M:养老型模式'
    
    # 产品名称包含"滚动持有期" → 滚动持有期模式
    if '滚动持有期' in fund_short_name:
        return 'G:滚动持有期'
...
def generate_param_mapping(std_data):
    ...
    mapping = {
        ...
        '募集开始日期': start_date if start_date else '',
        '募集结束日期': end_date if end_date else '',
        ...
        if param_name in ['账号', '开户银行', '大额支付号/SWIFT CODE', '账户标志']:
            ...
            for r in range(row - 1, 0, -1):
                ...
                if val and ('赎回款-' in str(val) or '分红款-' in str(val) or '认购退款-' in str(val)):
                    ...
                    mapped_key = f"{param_name}_{prefix}"
